...
import OutputHandler from './src/core/OutputHandler';
import SessionManager from './src/core/SessionManager';
import ContinuationHandler from './src/core/ContinuationHandler';
...
import IterativeCollaborativeOrchestrator from './src/orchestration/IterativeCollaborativeOrchestrator';
...
`);
  ...
  try {
    ...
    let templateMode: 'consensus' | 'orchestrated' | 'iterative' | null = null;
    let templateChunkSize: number | undefined;
    ...
    if (iterative) {
      ...
      await orchestrator.run(task as string, contextString);
      ...
      const session = sessionManager.createSessionManifest(
        'consensus',
        task as string,
        Object.values(conversationManager.agents),
 ...
 */
async function handleInitProject(args: string[]) {
 ...
 */
async function handleProjectInfo(args: string[]) {
  ...
  try {
    ...
    if (memory.coreContext.goals.length > 0) {
      console.log(`Goals:`);
      memory.coreContext.goals.forEach((g: string) => console.log(`  - ${g}`));
      console.log();
    }
    ...
    if (Object.keys(memory.metadata.agentParticipation).length > 0) {
      ...
      Object.entries(memory.metadata.agentParticipation).forEach(([agent, count]) => {
        console.log(`  ${agent}: ${count as number} conversations`);
      });
    }

    if (memory.decisions.length > 0) {
      console.log(`\nRecent Decisions (last 5):`);
      memory.decisions.slice(-5).forEach((d: any) => {
 ...
 */
async function handleDeleteProject(args: string[]) {
 ...
 */
async function handleListSessions(args: string[]) {
  const sessionManager = new SessionManager();
 ...
 */
async function handleShowSession(args: string[]) {
  const sessionManager = new SessionManager();
 ...
 */
async function handleDeleteSession(args: string[]) {
  const sessionManager = new SessionManager();
 ...
 */
async function handleResumeSession(args: string[]) {
  const sessionManager = new SessionManager();
  const continuationHandler = new ContinuationHandler();
  ...
  // Create judge
  const judge = {
    provider: ProviderFactory.createProvider(session.judge?.model || config.judge.model),
 ...
 */
function saveOrchestratedResults(result: any) {
  ...
  result.critiques.forEach((critique: any) => {
 ...
 */
function printOrchestratedSummary(result: any, filePaths: any) {
  console.log(`\n${'='.repeat(80)}`);
