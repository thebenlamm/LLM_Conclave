...
export default class InteractiveSession {
  ...
  constructor(config: any, projectId: string | null = null) {
    this.config = config;
    this.projectId = projectId;
    this.orchestrator = new Orchestrator(config, null); // MemoryManager is optional in Orchestrator constructor, passing null for now or update if needed
  ...
  async processMessage(message: string): Promise<void> {
    try {
      ...
      const result = await this.orchestrator.executeTask(message, null, {
        quiet: true,
        onStatus: (step: number, total: number, message: string) => {
      ...
      if (result.toolExecutions && result.toolExecutions.length > 0) {
        result.toolExecutions.forEach((exec: any) => {
          if (exec.success) {
