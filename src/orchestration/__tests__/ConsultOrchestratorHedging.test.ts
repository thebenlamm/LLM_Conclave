...
jest.mock('../../consult/artifacts/ArtifactExtractor', () => ({
  ArtifactExtractor: {
    extractIndependentArtifact: jest.fn((text, agentId) => ({ agentId, position: 'pos', keyPoints: ['pt'], rationale: 'rat', confidence: 0.9 })),
    extractSynthesisArtifact: jest.fn(() => ({ consensusPoints: [{ point: 'pt' }], tensions: [], priorityOrder: [] })),
    extractCrossExamArtifact: jest.fn(() => ({ challenges: [], rebuttals: [], unresolved: [] })),
    extractVerdictArtifact: jest.fn(() => ({ recommendation: 'rec', confidence: 0.9, evidence: [], dissent: [] }))
  }
}));
