...
export default class TaskClassifier {
   ...
   */
  static classify(task: string, availableAgents: string[] = []): any {
   ...
   */
  static getSecondaryAgents(primaryAgent: string, availableAgents: string[] = []): string[] {
   ...
   */
  static getDecisionMakers(availableAgents: string[] = []): string[] {
    // If we have AGENT_ROLES defined for these agents, filter by decision_maker type
    const rolesExist = availableAgents.some(name => AGENT_ROLES[name]);
   ...
   * @returns {Array<string>}
   */
  static getValidators(availableAgents: string[] = []): string[] {
