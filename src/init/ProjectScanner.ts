...
export default class ProjectScanner {
   ...
   */
  static async shouldScan(rl: readline.Interface): Promise<boolean> {
   ...
   */
  async scan(timeoutMs: number = 30000): Promise<any> {
   ...
   */
  async _detectProjectType(): Promise<void> {
    const indicators = [
   ...
   */
  async _detectNodeFramework(): Promise<string | null> {
    try {
   ...
   */
  async _detectPythonFramework(): Promise<string | null> {
    try {
   ...
   */
  async _detectPHPFramework(): Promise<string | null> {
    try {
   ...
   */
  async _detectRubyFramework(): Promise<string | null> {
    try {
   ...
   */
  async _analyzeStructure(): Promise<void> {
    try {
   ...
   */
  async _findKeyFiles(): Promise<void> {
    const keyFileNames = [
      'README.md',
      'README.txt',
      'package.json',
   ...
   */
  async _detectDomains(): Promise<void> {
    const domains = new Set<string>();

    // Check structure for domains
    const structureLower = this.results.structure.map(s => s.toLowerCase());

    if (structureLower.includes('api')) domains.add('API development');
    if (structureLower.includes('frontend') || structureLower.includes('client')) domains.add('Frontend');
    if (structureLower.includes('backend') || structureLower.includes('server')) domains.add('Backend');
    if (structureLower.includes('components')) domains.add('Component architecture');
    if (structureLower.includes('tests')) domains.add('Testing');
    if (structureLower.includes('docs')) domains.add('Documentation');
    ...
    if (readme) {
      ...
      if (content.includes('e-commerce') || content.includes('shop')) domains.add('E-commerce');
      if (content.includes('authentication') || content.includes('auth')) domains.add('Authentication');
      if (content.includes('database') || content.includes('db')) domains.add('Database');
      if (content.includes('api')) domains.add('API');
      if (content.includes('mobile')) domains.add('Mobile');
      if (content.includes('web app') || content.includes('webapp')) domains.add('Web application');
      if (content.includes('saas')) domains.add('SaaS');
      if (content.includes('analytics')) domains.add('Analytics');
   ...
   */
  _buildSummary(elapsed: string): string {
