...
export default class InteractiveInit {
  ...
  async run(): Promise<void> {
    try {
      ...
      if (!this.options.noScan && !this.options.scan) {
        ...
        if (this.rl && await ProjectScanner.shouldScan(this.rl)) {
  ...
  async promptProjectName(): Promise<string> {
    ...
    return new Promise((resolve) => {
      if (!this.rl) return resolve('my-project');
   ...
   */
  async scanProject(): Promise<string | null> {
    try {
   ...
   */
  async promptProjectDescription(): Promise<string> {
      ...
      const onLine = (line: string) => {
        if (line.trim() === '') {
          ...
          if (emptyLineCount >= 1) {
            ...
            if (this.rl) this.rl.off('line', onLine);
   ...
   */
  async presentAgentProposal(agents: any[], reasoning: string, generator: AgentGenerator): Promise<any[]> {
   ...
   */
  async showMenu(options: Record<string, string>): Promise<string> {
   ...
   */
  async showFullPrompts(agents: any[]): Promise<void> {
    console.log('\n' + '═'.repeat(80));
   ...
   */
  async finalizeSetup(projectName: string, agents: any[], description: string): Promise<void> {
    PromptBuilder.info('\nFinalizing setup...\n');
