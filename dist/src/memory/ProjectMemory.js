 ...
 */
class ProjectMemory {
    constructor(projectId, projectPath = null) {
     ...
     */
    updateAgentMemory(agentRole, knowledge) {
    ...
    addConversationReference(conversationRef) {
        ...
        (conversationRef.agents || []).forEach((agent) => {
    ...
    getRelatedConversations(topic, limit = 5) {
        return this.conversationReferences
            .filter(conv => conv.task.toLowerCase().includes(topic.toLowerCase()) ||
            conv.tags.some((tag) => tag.toLowerCase().includes(topic.toLowerCase())))
