...
class Orchestrator {
    ...
    async executeAgentWithTools(agent, messages, quiet = false) {
        ...
        while (iterations < maxIterations) {
            ...
            const response = await agent.provider.chat(currentMessages, agent.systemPrompt, { tools: agent.provider.getProviderName() === 'OpenAI' ? this.toolRegistry.getOpenAITools() : tools });
