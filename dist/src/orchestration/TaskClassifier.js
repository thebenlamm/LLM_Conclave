...
class TaskClassifier {
    ...
    static classify(task, availableAgents = []) {
        ...
        const hasRoleDefs = availableAgents.some(name => AgentRoles_1.AGENT_ROLES[name]);
        if (hasRoleDefs) {
            ...
            for (const agentName of availableAgents) {
                const role = AgentRoles_1.AGENT_ROLES[agentName];
                if (!role || role.type !== 'decision_maker')
                    continue;
                let score = 0;
                let matchedKeywords = [];
    ...
    static getSecondaryAgents(primaryAgent, availableAgents = []) {
        ...
        const rolesExist = availableAgents.some(name => AgentRoles_1.AGENT_ROLES[name]);
        if (rolesExist) {
            return Object.entries(AgentRoles_1.AGENT_ROLES)
                .filter(([name, role]) => availableAgents.includes(name) &&
    ...
    static getDecisionMakers(availableAgents = []) {
        ...
        if (rolesExist) {
            return Object.entries(AgentRoles_1.AGENT_ROLES)
                .filter(([name, role]) => availableAgents.includes(name) && role.type === 'decision_maker')
                .map(([name, _]) => name);
        }
    ...
    static getValidators(availableAgents = []) {
        ...
        const validatorsExist = availableAgents.some(name => AgentRoles_1.AGENT_ROLES[name] && AgentRoles_1.AGENT_ROLES[name].type === 'validator');
        if (validatorsExist) {
            return Object.entries(AgentRoles_1.AGENT_ROLES)
                .filter(([name, role]) => availableAgents.includes(name) && role.type === 'validator')
                .map(([name, _]) => name);
        }
...
exports.default = TaskClassifier;
